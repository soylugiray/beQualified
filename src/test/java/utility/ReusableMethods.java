...
public class ReusableMethods {
    ...
    public static void simulateHumanTyping(WebElement webElement, String input) throws InterruptedException {

        Random r = new Random();

        for (int i = 0; i < input.length(); i++) {
            try {
                Thread.sleep((int) r.nextGaussian() * 50 + 500);
    ...
    public static void clickOnCanvasCoordinates( WebElement canvas, int x, int y) {
        // Canvas üzerinde belirtilen koordinatlara tıklama
        Actions actions = new Actions(driver);
        actions.moveToElement(canvas, 0, 0).pause(5).moveByOffset(x, y).click(canvas).perform();
    }
...
//   waitFor(5);  => waits for 5 second => Thread.sleep(5000)
    ...
    */
    ...
    public static void yönDegistir(int x, int y , int x1 , int y1){
